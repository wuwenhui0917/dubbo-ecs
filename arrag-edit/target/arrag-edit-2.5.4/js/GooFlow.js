...
//初始化区域图的对象
    ...
    this.$trans=null;//事务方式
    this.$create=null;//创建人
    this.$desc=null;//创建人
...
//			this.$tool.append("<span/><a href='javascript:void(0)' type='group' class='GooFlow_tool_btn' id='"+this.$id+"_btn_group'><i class='ico_group'/></a>");
    ...
    if(this.$editable){
        this.$workArea.on("click",{inthis:this},function(e){
            ...
            This.addNode(This.$id+"_node_"+This.$max,{name:This.$id+"_node_"+This.$max,left:X,top:Y,type:This.$nowType});
            This.$max++;
        });
        //划线或改线时用的绑定
        ...
        //保存时调用的校验 格式为function（flow，node）
        this.onSaveCheck=null;
        this.onRightLayout=null;
...
//		    window.alert(JSON.stringify(this.$nodeData));
//			window.alert('right click'+id);
//
//	},
...
//		that =this
        ...
        this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
            ...
            var flam = e.data.inthis;


//			window.alert(flam.$nodeData[this.id]);


//			that.onRightClick(this.id);
...
//
//		this.$workArea.delegate(".ico","click",{inthis:this},function(e){
//			window.alert('dbclick')
//			var Dom=$(this).parents(".GooFlow_item");
//			e.data.inthis.onRightClick(Dom.id)
//
//		});
        ...
        this.$workArea.delegate(".GooFlow_item","mouseenter",{inthis:this},function(e){
        	console.log(">>>>>>>>>>>>workArea  mouseenter")
        ...
        this.$workArea.delegate(".GooFlow_item","mouseleave",{inthis:this},function(e){
            ...
            if(this.id==e.data.inthis.$focus){
            	console.log(">>>>>>>>>>>>>>.GooFlow_itemmouseleave$focus")
                $(this).css("border-color",GooFlow.prototype.color.line||"#3892D3");
            }else{
                console.log(">>>>>>>>>>>>>>.no    GooFlow_itemmouseleave$focus")
        ...
        this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
            ...
            if(lineStart&&!This.$mpTo.data("p")){
               // window.alert();
                This.addLine(This.$id+"_line_"+This.$max,{from:lineStart.id,to:this.id,name:This.$id+"_line_"+This.$max});
                This.$max++;
            }
            else{
                if(lineStart){
        ...
        this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
            ...
            This.$textArea.val(oldTxt).css({display:"block",width:$(this).width()+24,height:$(this).height(),
                ...
                .data("id",This.$focus).focus();
            // console.log('请'+This.$focus);
            This.$workArea.parent().one("mousedown",function(e){
                if(e.button==2)return false;
                // console.log("..........$workAreamousedown.7777777777777777=="+JSON.stringify(This.$textArea.data("id")))
    ...
    //取消所有结点/连线被选定的状态
    blurItem:function(){
    	console.log(">>>>>>>>>>>quxiao all blurItem")
        if(this.$focus!=""){
            var jq=$("#"+this.$focus);
    ...
    focusItem:function(id,bool){
        ...
        if(jq.prop("tagName")=="DIV"){
//			window.alert('fouce'+bool+JSON.stringify(this))
            if(bool&&this.onItemFocus!=null&&!this.onItemFocus(id,"node"))	{
                console.log('onItemFocues>>>>>>>>>>>>>>>>.');
    ...
    exportDataToJSON:function(){
        var ret={title:this.$title,nodes:[],links:[]};
        //node 信息
        for(var k1 in this.$nodeData){
            var node = this.$nodeData[k1];
            if(this.onSaveCheck){
            	if(!this.onSaveCheck(this,node)){
            		return ;
            	}
            }
            node.id=k1;
        ...
        for(var k2 in this.$lineData){
            var line = this.$lineData[k2];
            if(this.onSaveCheck){
            	if(!this.onSaveCheck(this,line)){
            		return ;
            	}
            }
            ret.links.push(line)
...
///////////以下为有关画线的方法
    ...
    drawPoly:function(id,sp,m1,m2,ep,mark){
        ...
        if(GooFlow.prototype.useSVG!=""){
            ...
            var hi=document.createElementNS("http://www.w3.org/2000/svg","path");
            var path=document.createElementNS("http://www.w3.org/2000/svg","path");
            if(id!="")	{poly.setAttribute("id",id);poly.setAttribute("name",id);}
